...
pub fn validate_path(root: &Path, file_path: &Path) -> Result<(), String> {
    ...
    if !canonical_parent.starts_with(&canonical_root) {
        return Err(format!("path escapes project root: {}", file_path.display()));
    }
    Ok(())
}
...
pub fn check_file_size(path: &Path, size_bytes: u64) -> bool {
    ...
    let text_extensions = [
        ...
        "cxx", "hpp", "md", "mdx", "txt", "rst", "toml", "yaml", "yml", "json", "xml",
        "rb", "swift", "kt", "scala", "php", "cs", "lua", "hs", "ex", "sh", "bash", "dart",
        "zig", "css", "html", "sql",
    ];
...
mod tests {
    ...
    fn check_file_size_allows_normal_files() {
        ...
        assert!(check_file_size(&PathBuf::from("main.py"), MAX_TEXT_FILE_SIZE));
        assert!(!check_file_size(&PathBuf::from("main.py"), MAX_TEXT_FILE_SIZE + 1));
    }

    #[test]
    fn check_file_size_allows_larger_binary() {
        use std::path::PathBuf;
        assert!(check_file_size(&PathBuf::from("image.png"), MAX_TEXT_FILE_SIZE + 1));
        assert!(!check_file_size(&PathBuf::from("image.png"), MAX_BINARY_FILE_SIZE + 1));
    }
}
