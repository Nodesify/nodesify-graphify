...
pub fn merge_graphs(
    ...
    std::fs::create_dir_all(&out_graphify).map_err(|e| napi::Error::from_reason(e.to_string()))?;
...
pub fn diff_graphs(root_a: String, root_b: String) -> napi::Result<DiffResultJs> {
    ...
    let edges_a: std::collections::HashSet<(String, String, String)> =
        collect_edges(&db_a).map_err(|e| napi::Error::from_reason(e.to_string()))?;
    let edges_b: std::collections::HashSet<(String, String, String)> =
        collect_edges(&db_b).map_err(|e| napi::Error::from_reason(e.to_string()))?;

    let added_edges = edges_b.difference(&edges_a).count() as i64;
    let removed_edges = edges_a.difference(&edges_b).count() as i64;

    let added_labels =
        query_labels(&db_b, &added_ids).map_err(|e| napi::Error::from_reason(e.to_string()))?;
    let removed_labels =
        query_labels(&db_a, &removed_ids).map_err(|e| napi::Error::from_reason(e.to_string()))?;
...
fn collect_ids(
    ...
    query: &str,
) -> graphify_core::Result<std::collections::HashSet<String>> {
...
fn collect_edges(
    db: &rusqlite::Connection,
) -> graphify_core::Result<std::collections::HashSet<(String, String, String)>> {
